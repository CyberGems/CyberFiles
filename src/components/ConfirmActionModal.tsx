      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
          <div className="flex items-center gap-2">
            ...
            <h2 id="confirm-delete-title" className="font-semibold text-neutral-100 text-sm">{title ?? t.core.deleteTitle}</h2>
          </div>
          <Tooltip label={t.core.cancel} placement="bottom"><button disabled={isBusy} onClick={onCancel} className="p-1 text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 rounded disabled:opacity-40"><X className="w-4 h-4" /></button></Tooltip>
        </div>

        <div className="p-4 space-y-3 text-sm">
          <p className="text-neutral-300">{description ?? t.core.deleteMessage}</p>
          {items.length > 0 && (
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-neutral-800 bg-neutral-950/50">
          <DialogButton size="compact" disabled={isBusy} onClick={onCancel}>{t.core.cancel}</DialogButton>
          <DialogButton size="compact" variant="danger" disabled={isBusy} onClick={onConfirm}>{isBusy ? busyLabel ?? t.core.operationInProgress : confirmLabel ?? t.core.delete}</DialogButton>
        </div>
      </section>
    </div>
  );
