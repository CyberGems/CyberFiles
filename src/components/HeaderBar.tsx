    ...
    <header className="min-h-14 bg-neutral-900/95 border-b border-neutral-800 px-3 py-2 flex items-center justify-between gap-3 select-none z-20 backdrop-blur-md">
      <div className="flex items-center gap-3 min-w-0">
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center shadow-lg shadow-cyan-500/20 text-black font-black text-sm tracking-tighter">CF</div>
          <div className="hidden lg:block">
            <div className="flex items-center gap-2">
              <span className="font-bold text-sm tracking-wide text-neutral-100">{t.app.title}</span>
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-neutral-800 text-neutral-400 border border-neutral-700/70">{t.app.stageBadge}</span>
            </div>
          </div>
        </div>
        ...
        <div className="header-actions flex items-center gap-1 overflow-x-auto min-w-0">
          ...
          <Tooltip label={t.sidebar.openLocalFolder}><button onClick={onOpenRealFolder} className="header-action accent"><FolderOpen className="w-3.5 h-3.5 text-cyan-400" /><span className="action-label">{language === 'es' ? 'Abrir carpeta' : 'Open folder'}</span></button></Tooltip>
        </div>
      </div>

      <div className="flex items-center gap-2 flex-shrink-0">
        <Tooltip label={t.header.switchLanguage}><button onClick={toggleLanguage} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-neutral-950 border border-neutral-700/80 hover:border-cyan-500 text-neutral-300 hover:text-cyan-300 transition-colors text-xs font-medium"><Globe className="w-3.5 h-3.5 text-cyan-400" /><span className={language === 'es' ? 'text-cyan-300 underline' : 'text-neutral-500'}>ES</span><span className="text-neutral-600">/</span><span className={language === 'en' ? 'text-cyan-300 underline' : 'text-neutral-500'}>EN</span></button></Tooltip>
        ...
        <Tooltip label={t.settings.openTooltip}><button onClick={onOpenSettings} className="p-1.5 rounded-md text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800 transition-colors" aria-label={t.settings.openTooltip}><Settings className="w-4 h-4" /></button></Tooltip>
        <Tooltip label={t.header.shortcutsTooltip}><button onClick={onOpenShortcuts} className="p-1.5 rounded-md text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800 transition-colors"><Keyboard className="w-4 h-4" /></button></Tooltip>
      </div>
